...
### Пакетный режим (вся папка сразу)

Если не хочется прописывать каждого респондента в `project.yaml`, оставь список пустым — программа сама найдёт все `.docx`/`.txt` файлы в папке `transcripts/`:

```yaml
respondents: []   # пусто → авто-обнаружение
```

Кинь все транскрипты в `transcripts/` и запусти как обычно. Идентификатор респондента берётся из имени файла (`interview_anna.docx` → `interview_anna`). Метаданные (сегмент, дата интервью) будут пустыми — при необходимости добавь респондентов в `project.yaml` позже.
...
# Список респондентов.
# Если список пустой (respondents: []) — программа автоматически
# найдёт все .docx/.txt файлы в transcripts/ и обработает их.
respondents:
  - id: R01
